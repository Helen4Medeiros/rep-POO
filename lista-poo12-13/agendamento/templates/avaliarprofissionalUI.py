import streamlit as st
from views import View

class AvaliarProfissionalUI:
    def main():
        st.header("Avaliar Profissionais")
        id_cliente = st.session_state["usuario_id"]
        
        profissionais = View.avaliacao_listar_profissionais_para_avaliar(id_cliente)
        if not profissionais:
            st.info("Você não tem profissionais para avaliação no momento (aguardando confirmação ou já avaliados).")
        else:
            profissional = st.selectbox('Informe o profissional: ', profissionais)
        
        # for p in profissionais:
        #     opcoes_exibicao.append(f"{p['nome']} ({p['especialidade']})")
        
        # indice_selecionado = st.selectbox(
        #     "Profissionais com agendamento confirmado e ainda não avaliados:",
        #     options=opcoes_exibicao, 
        #     index=0 
        # )
        
        # if indice_selecionado > 0:
        #     profissional_selecionado = profissionais[indice_selecionado - 1]
            
        #     id_profissional = profissional_selecionado['id']
        #     nome_profissional = profissional_selecionado['nome']
            
        #     st.subheader(f"Avaliar {nome_profissional}")

        #     form_key = f"avaliacao_form_{id_profissional}" 
            
        #     with st.form(key=form_key):
        #         nota = st.slider("Nota (0 estrela é ruim, 5 estrelas é excelente)", 0, 5, 0) 
        #         comentario = st.text_area("Comentário (Opcional):")
        #         submit_button = st.form_submit_button(label=f"Enviar Avaliação para {nome_profissional}")

        #         if submit_button:
        #             try:
        #                 View.avaliacao_inserir(nota, comentario, id_cliente, id_profissional)
        #                 st.success(f"Avaliação enviada com sucesso para {nome_profissional}! Obrigado.")
        #                 st.balloons()
        #                 st.rerun() 
        #             except ValueError as e:
        #                 st.error(f"Erro ao enviar avaliação: {e}")
        #             except Exception as e:
        #                 st.error(f"Ocorreu um erro interno: {e}")